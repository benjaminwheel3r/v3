...
```{r}
lm.global <- lm(Candidate_VS~Candidate_TS18+Candidate_TS19b4+Candidate_TS17+Perc_BachelorsDeg_and_higher+perc_Fem+MedianAge_Total+PopDens, data = spdf_co2)
summary(lm.global)
...
```{r}
f1 <- Candidate_VS~Candidate_TS18+Candidate_TS19b4+Candidate_TS17+Perc_BachelorsDeg_and_higher+perc_Fem+MedianAge_Total+perc_white+PopDens
m1 <- lm(f1, data = spdf_co2)
summary(m1)

lm.global <- lm(Candidate_VS~Candidate_TS18+Candidate_TS19b4+Candidate_TS17+Perc_BachelorsDeg_and_higher+perc_Fem+MedianAge_Total+perc_white+PopDens, data = spdf_co2)
summary(lm.global)
lm.global$residuals
```
...
bw.gwr.1 <- bw.gwr(Candidate_VS~Candidate_TS18+Candidate_TS19b4+Candidate_TS17+Perc_BachelorsDeg_and_higher+perc_Fem+MedianAge_Total+perc_white+PopDens, data = spdf_co2, approach = "AICc",
	kernel = "bisquare", adaptive = TRUE, longlat = TRUE)
bw.gwr.2 <- bw.gwr(Candidate_VS~Candidate_TS18+Candidate_TS19b4+Candidate_TS17+Perc_BachelorsDeg_and_higher+perc_Fem+MedianAge_Total+PopDens+perc_white, data = spdf_co2, approach = "cv", kernel = "bisquare", adaptive=TRUE)
bw.gwr.1
bw.gwr.2
...
gwr.res <- gwr.basic(Candidate_VS~Candidate_TS18+Candidate_TS19b4+Candidate_TS17+Perc_BachelorsDeg_and_higher+perc_Fem+MedianAge_Total+PopDens+perc_white,
...
spplot(gwr.res$SDF, "Candidate_TS18", key.space = "right", 
	main = "Basic GW regression coefficient estimates for Candidate_TS18",
	sp.layout = map.layout)

spplot(gwr.res$SDF, "perc_white", key.space = "right", 
	main = "Basic GW regression coefficient estimates for perc_white",
	sp.layout = map.layout)
```

```{r}
setwd("C:/Temp/FINAL_INPUTS/Box Sync/R/v4/GWR/GWR_Analysis/Results_Bivariate/Images/FINAL_MODEL/wboth_borders")
sdf <- gwr.res$SDF
...
for (i in colm)
{
val <- sdf[eval(parse(text=paste("abs(sdf@data$",i,"_TV) > 1.67",sep =''))),]
...
y = tm_shape(sdf) +tm_fill(col = "y", breaks = est, palette = c('white','grey','black')) +  tm_legend(outside = FALSE, text.size = .8)+tm_borders()
save_tmap(tm = y, filename = paste("y",suffix,".png",sep = ''))
y

yhat = tm_shape(sdf) +tm_fill(col = "yhat", breaks = est, palette = c('white','grey','black')) +  tm_legend(outside = FALSE, text.size = .8)+tm_borders()
save_tmap(tm = yhat, filename = paste("yhat",suffix,".png",sep = ''))
yhat

StudRes= tm_shape(sdf) +tm_fill(col = "Stud_residual") +  tm_legend(outside = FALSE, text.size = .8)+tm_borders()
save_tmap(tm = StudRes, filename = paste("StudRes",suffix,".png",sep = ''))
StudRes

res= tm_shape(sdf) +tm_fill(col = "residual",breaks = c(-Inf,-0.05, -0.02,-.01, 0, 0.01, 0.02, 0.05, Inf)) +  tm_legend(outside = FALSE, text.size = .8)+tm_borders()
save_tmap(tm = res, filename = paste("res",suffix,".png",sep = ''))
res

res_ols = tm_shape(sdf) +tm_fill(col = "residual_ols") +  tm_legend(outside = FALSE, text.size = .8)+tm_borders()
save_tmap(tm = res_ols, filename = paste("residuals_ols",suffix,".png",sep = ''))
res_ols

yhat_ols = tm_shape(sdf) +tm_fill(col = "yhat_ols", breaks = est, palette = c('white','grey','black')) +  tm_legend(outside = FALSE, text.size = .8)+tm_borders()
save_tmap(tm = yhat_ols, filename = paste("yhat_ols",suffix,".png",sep = ''))
yhat_ols
